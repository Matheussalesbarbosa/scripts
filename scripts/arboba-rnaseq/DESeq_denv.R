...
countdata <- read.table("D:\\RNASeqArbovirusFiocruzBA\\analysis\\counts\\counts-d\\counts.txt", header=TRUE, row.names=1)
...
(condition <- factor(c(rep("denv", 8), rep("ctrl", 9))))
...
rld_pca(rld, colors=mycols, intgroup="condition", xlim=c(-35, 50))
dev.off()
